...
export const Toolbar = () => {
    ...
    // Close dropdown when focus leaves the toolbar area
    useEffect(() => {
        const handleFocusOut = (event) => {
            if (openDropdown && toolbarRef.current) {
                // Check if the new focus target is outside the toolbar
        ...
        if (toolbarRef.current) {
            toolbarRef.current.addEventListener('focusout', handleFocusOut);
            return () => {
                if (toolbarRef.current) {
                    toolbarRef.current.removeEventListener('focusout', handleFocusOut);
                }
            };
        }
    }, [openDropdown]);

    // Close dropdown when Tab reaches the end of dropdown options
    useEffect(() => {
        const handleKeyDown = (event) => {
            if (event.key === 'Tab' && openDropdown && toolbarRef.current) {
                const activeElement = document.activeElement;
                const currentDropdown = toolbarRef.current.querySelector(`[data-dropdown="${openDropdown}"]`);
                
                if (currentDropdown && currentDropdown.contains(activeElement)) {
                    // Check if we're at the last focusable element in the dropdown
                    const focusableElements = currentDropdown.querySelectorAll('[tabindex="0"]');
                    const lastElement = focusableElements[focusableElements.length - 1];
                    
                    if (activeElement === lastElement && !event.shiftKey) {
                        // Tab from last element - close dropdown and move to next toolbar button
                        setOpenDropdown(null);
                        // Focus will naturally move to next element
                    }
                }
            }
        };
        ...
        if (toolbarRef.current) {
            ...
            return () => {
                if (toolbarRef.current) {
                    toolbarRef.current.removeEventListener('keydown', handleKeyDown);
                }
            };
        }
    }, [openDropdown]);
    ...
    return (
        <div ref={toolbarRef} className="tw-builder__toolbar" role="toolbar" aria-label="Element toolbar">
            <div 
                ...
                onMouseEnter={() => setOpenDropdown('layout')}
            ...
            >
                ...
                <div 
                    className={`tw-builder__toolbar-dropdown ${openDropdown === 'layout' ? 'tw-builder__toolbar-dropdown--open' : ''}`}
                    role="menu" 
                    aria-label="Layout elements menu" 
                    onMouseEnter={() => setOpenDropdown('layout')}
                    onMouseLeave={() => setOpenDropdown(null)}
                >
                    <div className="tw-builder__toolbar-dropdown-column">
                        ...
                        onKeyDown={(e) => {
                            if (e.key === 'Enter' || e.key === ' ') {
                                ...
                                e.stopPropagation();
                                createElement("block");
                                // Don't close dropdown - keep it open like click behavior
                            }
                        }}
                        tabIndex={openDropdown === 'layout' ? 0 : -1}
                        role="menuitem"
            ...
            <div 
                ...
                data-dropdown="texts"
                onMouseEnter={() => setOpenDropdown('texts')}
                onMouseLeave={() => setOpenDropdown(null)}
                onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') {
                        e.preventDefault();
            ...
            >
                ...
                <div 
                    className={`tw-builder__toolbar-dropdown ${openDropdown === 'texts' ? 'tw-builder__toolbar-dropdown--open' : ''}`}
                    role="menu" 
                    aria-label="Text elements menu" 
                    onMouseEnter={() => setOpenDropdown('texts')}
                    onMouseLeave={() => setOpenDropdown(null)}
                >
                    <div className="tw-builder__toolbar-dropdown-column">
                        ...
                        tabIndex={openDropdown === 'texts' ? 0 : -1}
                        role="menuitem"
            ...
            <div 
                ...
                onMouseEnter={() => setOpenDropdown('cookies')}
                onMouseLeave={() => setOpenDropdown(null)}
                onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') {
                        e.preventDefault();
            ...
            >
                ...
                <div 
                    className={`tw-builder__toolbar-dropdown ${openDropdown === 'cookies' ? 'tw-builder__toolbar-dropdown--open' : ''}`}
                    role="menu" 
                    aria-label="Cookie elements menu" 
                    onMouseEnter={() => setOpenDropdown('cookies')}
                    onMouseLeave={() => setOpenDropdown(null)}
                >
                    <div className="tw-builder__toolbar-dropdown-column">
                        ...
                        tabIndex={openDropdown === 'cookies' ? 0 : -1}
                        role="menuitem"
