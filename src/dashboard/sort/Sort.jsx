...
export const Sort = ({ onSortChange }) => {
  ...
  const [ascending, setAscending] = useState(true); // State to store ascending or descending order

  // Switch between alphabetical <-> date
  const toggleSortMode = () => {
    const newMode = sortMode === 'alphabetical' ? 'date' : 'alphabetical';
    setSortMode(newMode);
    onSortChange(newMode, ascending);
  };

  // Switch between ascending <-> descending
  const toggleDirection = (e) => {
    e.stopPropagation(); // Prevent triggering toggleSortMode
    const newDirection = !ascending;
    setAscending(newDirection);
    onSortChange(sortMode, newDirection);
  };

  const sortLabel = sortMode === 'alphabetical' ? 'A-Z' : 'Date';
  ...
  return (
    <button className="sort__button" onClick={toggleSortMode}>
      Sort: {sortLabel}
      <span className="sort__icon" onClick={toggleDirection}>
        {ascending ? (
            '↓' ) : (
            '↑'
            )}
